...
flowchart TD
    ...
    Open --> Click[📋 신규 수임처 클릭]
    Click --> NewBtn[+ 신규 등록 클릭]
    ...
    FormError --> Form
    Validate -->|통과| Save[(new-clients.json<br/>checklist: {} 초기화)]
    Save --> SlackNotify[Slack #신규수임처 카드 알림]
    SlackNotify --> Toast[✅ 등록 완료 토스트]
    Toast --> List[목록 페이지 진입<br/>ABC회사 진행률 0/19]

    List --> Detail[상세 페이지 진입]
    Detail --> Branch{이관여부 &<br/>사업자 생성여부<br/>분기}

    Branch --> Checklist[19개 항목 수동 체크 진행]
    Checklist -->|각 편집| PATCH[PATCH /api/new-client/:id/<br/>checklist/:itemKey]
    PATCH --> UpdateFile[(JSON 파일 갱신<br/>updatedAt 기록)]
    UpdateFile --> ReloadList[목록 진행률 재로드]
    ReloadList --> Check{19/19<br/>완료?}
    Check -->|No| Checklist
    ...
    style Save fill:#e1e8ff
    style UpdateFile fill:#e1e8ff
```
...
flowchart LR
    ...
    TN --> TR2[transferData<br/>신규 → 완료 처리]
    ...
    TR1 --> Join[공통 체크리스트 진행<br/>홈택스수임, EDI, CMS,<br/>사업용계좌·카드, 현영가맹점,<br/>위멤버스, 세모리포트 등]
    TR2 --> Join
    BR1 --> Join
    BR2 --> Join
...
flowchart TB
    ...
    Combo -->|이관 + 신규생성| C2[⚠️ 개업 중간 이관<br/>드물지만 가능]
...
sequenceDiagram
    ...
    U->>UI: 체크박스 클릭 / 드롭다운 / 인풋 blur
    UI->>Hook: update(itemKey, { status/value/note })
    Hook->>API: PATCH /api/new-client/:id/checklist/:itemKey
    API->>V: validateChecklistUpdate(itemKey, body)
    alt unknown item
        V-->>API: { ok: false, status: 400 }
        API-->>Hook: 400 error
        ...
        V-->>API: { ok: true, def, payload }
        API->>S: updateChecklistItem(file, id, itemKey, payload, kind)
        S->>FS: readFile
        FS-->>S: records[]
        alt record 없음
            S-->>API: null
            ...
            S->>S: checklist[itemKey] = { ..., updatedAt: now }
...
flowchart TB
    ...
    style Layer0 fill:#e1f5e1
    style Layer1 fill:#e1e8ff
    style Layer2 fill:#e1e8ff
    style Layer3 fill:#fff4e1
    style Layer4 fill:#ffe1d0
