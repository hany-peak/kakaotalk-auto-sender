...
/**
 * 1. Inputs에 Name과 Value를 각각 먼저 설정한다.
 * 2. 아래 config에서 해당 Input값을 상수에 할당한다.
 */
...
const TARGET_BASE_ID = 원천세_2025년_베이스ID;
...
try {
    ...
    console.log(`✅ Upsert 성공! 총 ${data.records.length}개의 레코드가 처리되었습니다.`);
    console.log("처리된 레코드 정보:", data.records);

    // 스크립트 결과를 다음 Automation 단계로 넘기기 위해 출력
    output.set('upsertedRecords', data.records);

} catch (error) {
    console.error('다른 베이스 접근 중 오류 발생:', error.message);
    // 오류 발생 시 Automation을 중단하거나 알림을 보낼 수 있습니다.
}